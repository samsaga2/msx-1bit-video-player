...
def compress(values):
    def equal_byte(left, right, length):
        return left < right \
    ...
    def equal_len(left, right):
        length = 0
        while equal_byte(left, right, length):
            length += 1
            left += 1
            right += 1
        return length
    ...
    while i < len(values):
        ...
        if back_len==0:
            blocks.append([back_len, next_char])
        else:
            blocks.append([back_len, back_offset, next_char])
        i += 1
    ...
    out = sum(blocks,[])
    sys.stderr.write(
        "original %s bytes -> final %s bytes\n" % (
            len(values), len(out)))
    block_size = len(blocks)
    return [block_size&0xff,block_size>>8] + out
